...
# start.hour <- "19:00"
# end.hour <- "23:00"
# met <- "/home/thalles/hysplit-july-2013/"
...
# tz <- "GMT"
# clean.files <- TRUE
# meteorological.data.archives <- "gdas"
# met.file.name <- "gdas1.jul13.w2"
# lat <- 48.466667
# lon <- -67.433333
...
ProcTraj <- 
  function(lat = 51.5, lon = -45.1,
                     ...
                     tz="GMT", meteorological.data.archives = "reanalysis", 
                     met.file.name = NULL, clean.files=TRUE ) {
  ...
  hy.split.exec.dir <- normalizePath(hy.split.exec.dir)
  ...
  bdyfiles.path <- normalizePath(bdyfiles.path)
  ...
  for (i in 1:length(dates.and.times)) {
    ...
    if (identical( meteorological.data.archives, "reanalysis")) {
      met.file.name <- "RP"
      
      line <- paste("echo 3 >> ", control.file, "\n", sep="")
      cat(line, file = script.file, sep = "\n")
      ...
      for (i in 1:3) {
        AddMetFiles(met.file.name, months[i], Year, met, script.file, control.file)
      }
    }
    
    if (identical( meteorological.data.archives, "gdas")) {
      line <- paste("echo 1 >> ", control.file, "\n", sep="")
      cat(line, file = script.file, sep = "\n")
      ...
      line <- paste("echo", met.file.name, ">>", control.file, sep = " ")
      cat(line, file = script.file, sep = "\n")
    }
    
    ## Directory location to which the text trajectory end-points file will be written. 
    ## Always terminate with the appropriate slash (\ or /).
  ...
  if( !missing(out) ) {
    ...
    file.name <- paste(out, name, Year, "-", month, "-", day, "-", height, "m.RData", sep = "")
